...
- [x] 2.2 裂变分支（`step.iterate_open_tabs === true` 且非最后一步）：主 tab SCAN 快照描述符（text/href/aria/title/index）→ 创建 1 个临时 tab（自动激活）→ 逐元素 [加载列表页 → 描述符优先匹配 + 索引兜底 → 点击 → confirm → 临时 tab 内执行后续步骤 → 主 tab MARK 视觉标记 → 元素间隔/组间休息] → 关闭临时 tab 激活主 tab
