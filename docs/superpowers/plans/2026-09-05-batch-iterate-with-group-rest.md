...
- [ ] **Step 1: 在 `#iterate-interval-group` 容器内追加两个输入框**
...
- [ ] **Step 3: `editStep()` 回填新字段**

在 `electron/renderer/index.html` 的 `editStep()` 内、`step-iterate-interval` 回填行（约 L1179）后追加两行：

```js
            document.getElementById('step-iterate-batch-size').value = step.iterate_batch_size || 10;
            document.getElementById('step-iterate-batch-interval').value = step.iterate_batch_interval || 60;
```

- [ ] **Step 4: step-form submit 读取新字段**
...
```js
            ...
            const iterate_batch_size = parseInt(document.getElementById('step-iterate-batch-size').value, 10) || 10;
            const iterate_batch_interval = parseInt(document.getElementById('step-iterate-batch-interval').value, 10) || 60;

            const step = { url, button_selectors, confirm_selectors, iterate_all, iterate_interval, iterate_batch_size, iterate_batch_interval };
```

- [ ] **Step 5: `renderSteps()` 卡片展示分批标记**
